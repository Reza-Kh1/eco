...
export default function Facilities() {
  ...
  const deleteTodo = (id: string) => {
    axios.delete(`facilities/deleteFacility/${id}`).then(() => {
      toast.success("رکورد حذف شد")
      getList()
    }).catch((err) => { toast.warning("حذف با خطا روبرو شد"), console.log(err) }
    )
  }
  const createTodo = (id: string, years: string) => {
    ...
    axios.post("facilities/postFacility", body)
      .then(() => { toast.success("رکورد افزوده شد"), deleteTable(id), getList() })
      .catch(() => { toast.error("اطلاعات ذخیره نشدند") })
  }
  const saveTodo = (id: string, years: string) => {
    ...
    console.log(body);
    axios.put(`facilities/updateFacility/${id}`, body).then(() => { toast.success("رکورد آپدیت شد"), getList(), setEditId("") }).catch((err) => {
      console.log(err)
  ...
  const createTable = () => {
    ...
    for (let i = 0; i < value; i++) {
      ...
      let length = { name: "", title: "", tashilat: "", motevali: "", date: year, quantity: "", id: id.toFixed() }
      body.push(length)
    }
    setDemoTable(body)
    setValue("number", "")
  ...
  const verifyEval = () => {
    axios.get(`users/sendToEval/${selectFormRegister?.companyId}`).then(() => {
  ...
  const getList = () => {
    if (!selectFormRegister?.companyId) return
    axios.get(`facilities/allFacilitiesOfCompany/${selectFormRegister?.companyId}`).then(({ data }) => {
      if (data.flags) {
  ...
  return (
    <div className="w-full min-h-80 flex flex-col justify-between">
      <div>
        <span className="text-2xl mt-2 text-blue-400 text-center mb-5 block">تسهیلات</span>
        <div className="w-full flex justify-evenly items-end mb-3">
          <div className="w-2/6">
            ...
            <div className="w-full mt-2" style={{ direction: "ltr" }}>
              <Select
                ...
                value={year}
              >
                {selectFormRegister.listLimitedYear && selectFormRegister.listLimitedYear.map((i, index) => (
          ...
          <Button onClick={createTable} variant="gradient" className="w-1/6" color="blue">
            افزودن
        ...
        <div className="flex flex-col gap-2">
          ...
          {allData.length && allData.map((i, index) => (
            <div key={index} className="w-fullf flex flex-col bg-blue-gray-100 p-2 rounded-md">
              <div className="flex justify-between items-center">
                <span>سال : {Number(i.date).toLocaleString("fa").replace(/٬/g, "")}</span>
                {i?.evalApproved === true ? (
                  <span className="rounded-md px-4 text-xs text-gray-900 py-2 bg-green-300">تایید شده</span>
                ) : i?.evalApproved === false ? (
                  <span className="rounded-md px-4 text-xs text-gray-900 py-2 bg-red-100">تایید نشده</span>
                ) : (
                  <span className="rounded-md px-4 text-xs text-gray-900 py-2 bg-gray-200">ثبت نشده</span>
                )}
                <div>

                  <ShowMsg value={i.msg} />
                  <Button variant="gradient" color="red" className="px-5 font-medium ml-2 py-2" onClick={() => deleteTodo(i.id)}>
                    حذف
                  </Button>
                  {editId === i.id ? (
                    <Button variant="gradient" color="green" className="px-5 font-medium py-2" onClick={() => saveTodo(i.id, i.date)}>
                      ذخیره
                    </Button>
                  ) : (
                    <>
                      {!i.companyId ? (
                        <Button variant="gradient" color="blue-gray" className="px-5 font-medium py-2" onClick={() => createTodo(i.id, i.date)}>
                          ثبت
                        </Button>
                      ) : (
                        <Button variant="gradient" color="blue" className="px-5 font-medium py-2" onClick={() => editTodo(i)}>
                          ویرایش
                        </Button>
                      )}
                    </>
              ...
              {editId === i.id || !i.companyId ? (
                <div className="flex flex-wrap justify-between items-center mt-3">
                  ...
                  <div className="w-3/12">
                    ...
                    <div className="w-full p-1" style={{ direction: "ltr" }}>
                      <Select
                        className=" min-w-36"
                        onChange={(value: string) => setFacSelectTodo(value)}
                        label="تسهیلات"
                        value={facSelectTodo}
                      >
                  ...
                  <div className="w-3/12 p-1">
                    <span className="block mb-2 text-sm">مبلغ یا تعداد :</span>
                    <InputForm type="text"
                      register={register}
                ...
                <div className="flex flex-wrap justify-between items-center mt-3">
                  ...
                  <div className="w-3/12 p-1">
                    ...
                    <span className="block px-3 py-1 rounded-md bg-gray-50 mt-2">
                      {i?.type === 1 ? "مالی مستقیم" : i?.type === 2 ? "مالی غیر مستقیم" : i?.type === 3 ? "خدمت" : i?.type === 4 ? "تسهیلگری" : "غیرمالی (سایر)"}
                    </span>
                  </div>
                  <div className="w-3/12 p-1">
                    <span className="text-sm">تعداد یا مبلغ :</span>
                    <span className="block px-3 py-1 rounded-md bg-gray-50 mt-2">
                      {i?.tedad ? Number(i.tedad).toLocaleString("fa") : Number(i?.quantity).toLocaleString("fa")}
                    </span>
                  </div>
                </div>
              )}
          ...
          {demoTable.length ? demoTable.map((i, index) => (
            <div key={index} className="w-fullf flex flex-col bg-blue-gray-100 p-2 rounded-md">
              <div className="flex justify-between items-center">
                <span>سال : {Number(i.date).toLocaleString("fa").replace(/٬/g, "")}</span>
                <span className="rounded-md px-4 text-xs text-gray-900 py-2 bg-gray-200">ثبت نشده</span>
                <div>
                  <Button variant="gradient" color="red" className="px-5 font-medium ml-2 py-2" onClick={() => deleteTable(i.id)}>
                    حذف
                  </Button>
                  <Button variant="gradient" color="green" className="px-5 font-medium py-2" onClick={() => createTodo(i.id, i.date)}>
                    ذخیره
                  </Button>
                </div>
              </div>
              <div className="flex flex-wrap justify-between items-center mt-3">
                ...
                <div className="w-3/12">
                  ...
                  <div className="w-full p-1" style={{ direction: "ltr" }}>
                    <Select
                      className=" min-w-36"
                      onChange={(value: string) => setFacSelect(value)}
                      label="تسهیلات"
                      value={""}
                    >
                      <Option value={"1"}>مالی مستقیم</Option>
      ...
      <div className="w-full flex justify-between mt-5">
        <div className="w-2/12">
          <Link to={"/admin/form-register/sales"}>
            <SubmitBtn type="button" value="صفحه قبلی" iconRight={<FaArrowRightLong className="inline ml-3" />} classPlus={"w-full"} />
          </Link>
        </div>
        <div className="w-2/12">
          <Button color="green" onClick={verifyEval} className="w-full" variant="gradient">
            <span>
              ارزیابی فرم
            </span>
