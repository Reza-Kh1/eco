...
export default function DaneshBonyan() {
  ...
  const createhandler = (time: string) => {
    ...
    axios.post("knowledge/postKnowledgeSale", body).then(() => {
  ...
  const editHandler = (num: number, id: string) => {
    ...
    axios.put(`knowledge/updateKnowledgeSale/${id}`, body).then(() => { toast.success("با موفقیت ذخیره شد"), getList(), setEditId(null) }).catch(() => toast.error("اطلاعات ذخیره نشد"))
  }
  const getData = async () => {
    let data: any
  ...
  const getList = async () => {
    ...
    axios.get(`knowledge/allKnowledgeSalesOfCompany/${selectFormRegister?.companyId}`).then(({ data }) => {
      if (data.flags) {
  ...
  return (
    <div className="w-full min-h-80 flex flex-col justify-between">
      <div>
        <span className="text-2xl mt-2 text-blue-400 text-center mb-5 block">دانش بنیان</span>
