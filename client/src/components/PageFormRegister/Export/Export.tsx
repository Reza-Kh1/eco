...
export default function Export() {
  ...
  const createhandler = (time: string) => {
    ...
    axios.post("exports/postExport", body).then(() => {
  ...
  const editHandler = (num: number, id: string) => {
    ...
    axios.put(`exports/updateExport/${id}`, body).then(() => { toast.success("با موفقیت ذخیره شد"), getList(), setEditId(null) }).catch(() => toast.error("اطلاعات ذخیره نشد"))
  }
  const getData = async () => {
    let data: any
  ...
  const getList = async () => {
    ...
    axios.get(`exports/allExportsOfCompany/${selectFormRegister?.companyId}`).then(({ data }) => {
      if (data.flags) {
  ...
  return (
    <div className="w-full min-h-80 flex flex-col justify-between">
      <div>
        <span className="text-2xl mt-2 text-blue-400 text-center mb-5 block">صادرات</span>
      ...
      <div className="w-full flex justify-between mt-5">
        <div className="w-2/12">
          <Link to={"/admin/form-register/users"}>
