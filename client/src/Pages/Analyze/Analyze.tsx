...
export default function Analyze() {
  ...
  const gog =
    role?.type === 0 || role?.type === 5
      ? "sales/salesOfCompanyByUserId"
      : "sales/salesOfCompany";
  const [chart, setChart] = useState<string>(query.get("chart") || gog);
  const [choiceCompany, setChoiceCompany] = useState<
  ...
  const userChart = () => {
    console.log(role?.id);
    axios
      .get(`${chart}/${yearSelector.in}/${yearSelector.to}/${role?.id}`)
      .then(({ data }) => {
        setDataChart([data]);
      })
      .catch((err) => {
  ...
  const getFilterChart = async () => {
    ...
    if (!yearSelector.in || !yearSelector.to || !chart || !choiceCompany.length)
      return;
    setLoading(true);
    try {
      const value = await Promise.all(
        choiceCompany.map(async (i) => {
          const { data } = await axios.get(
            `${chart}/${yearSelector.in}/${yearSelector.to}/${i.id}`
          );
          return data;
        })
      );
  ...
  const filterCompany = (e: any) => {
    const gog = getCompany.map((i) => {
      if (i.id === e.target.id) {
        if (i.status !== true) {
          if (
            choiceCompany.length > 4 ||
            (chart === "graph" && choiceCompany.length > 0)
          ) {
          ...
          i.status = !i.status;
          const newChocieCompany = choiceCompany.filter(
            (item) => item.id !== i.id
          );
  ...
  const getListCompany = () => {
    ...
    axios
      .get("company/getAllComany?skip=1&take=300")
      .then(({ data }) => {
  ...
  const getGraphData = async () => {
    ...
    const url = [
      "graph/getAllConnections/" + yearSelector.in,
      `graph/getAllNodesOfCompanies/${choiceCompany[0].name}/${yearSelector.in}`,
    ];
  ...
  return (
    <>
      ...
      >
        <div
          className="w-9/12 p-1 max-h-screen"
              ...
              <div className="h-full">
                {dataChart ? (
                  <Suspense fallback={<div>loading ...</div>}>
                    {chart === "graph" ? (
                      <div className="w-full h-full overflow-hidden bg-gray-100 border-blue-gray-800 border rounded-md shadow-md">
                        {" "}
                        <GraphChart graph={dataChart} />
                      </div>
                    ) : (
        ...
        <div className="w-3/12 p-1 min-h-full">
          <div
            className="w-full flex justify-around p-2 shadow-md bg-gray-50 dark:bg-gray-800 rounded-md min-h-full flex-wrap my-class overflow-y-auto overflow-hidden"
            ...
            <div className="flex flex-col w-full gap-2">
              {role?.type === 0 || role?.type === 5 ? (
                <div style={{ direction: "ltr" }}>
                  <Select
                    placeholder
                    onChange={(value: any) => setChart(value)}
                    label="انتخاب نمودار"
                    animate={{
                      mount: { y: 0 },
                      unmount: { y: 25 },
                  ...
                  >
                    ...
                    <Option value="knowledge/knowledgeSalesOfCompanyByUserId">
                      فروش دانش بنیان
                    </Option>
                    <Option value="exports/exportsOfCompanyByUserId">
                      صادرات
                    </Option>
                    <Option value="employees/employeesOfCompanyByUserId">
                      کارمندان
                    </Option>
                    <Option value="facilities/facilitiesOfCompanyByUserId">
                      تسهیلات مالی
                    </Option>
                  </Select>
                </div>
                ...
                <div style={{ direction: "ltr" }}>
                  ...
                  >
                    ...
                    <Option value="knowledge/knowledgeSalesOfCompany">
                      فروش دانش بنیان
                    </Option>
                    <Option value="exports/exportsOfCompany">صادرات</Option>
                    <Option value="employees/employeesOfCompany">
                      کارمندان
                    </Option>
                    <Option value="facilities/facilitiesOfCompany">
                      تسهیلات مالی
                    </Option>
              ...
              <div style={{ direction: "ltr" }}>
                <Select
                  placeholder
                  onChange={(value: any) => {
                    setYearSelector({ ...yearSelector, in: value });
                  }}
                  label="از سال"
                  animate={{
                    mount: { y: 0 },
              ...
              {chart !== "graph" && (
                <div style={{ direction: "ltr" }}>
                  <Select
                    ...
                    className="text-span-light dark:text-span-dark"
                    onChange={(value: any) => {
                      setYearSelector({ ...yearSelector, to: value });
                    }}
              ...
              {userData?.type !== 0 && choiceCompany.length ? (
                <div className="w-full shadow-md rounded-md dark:shadow-[#5c86c4] dark:bg-[#4e5255] p-2 flex flex-wrap gap-2">
                  <span className="text-span-light dark:text-span-dark w-full">
                    شرکت های فیلتر شده
                  </span>
                  {getCompany.length &&
                    getCompany.map((i) => {
                      {
                        if (i.status) {
                          return (
                            <button
                              onClick={() => deleteCompany(i.id)}
                              key={i.id}
                              className="px-3 py-1 rounded-md inline-block cursor-pointer text-gray-50 bg-blue-500 shadow-md"
                            >
                              {i.name} <IoClose className="inline" />
                            </button>
                          );
                        }
                      }
              ...
              {userData?.type !== 0 && (
                ...
                >
                  <AccordionHeader
                    placeholder={""}
                    className={
                      "text-base text-span-light dark:text-span-dark font-medium dark:shadow-gray-700 border-none dark:bg-[#4e5255] shadow-md pr-2 rounded-lg py-2"
                    }
                  ...
                  <AccordionBody>
                    <Card placeholder={""}>
                      <List
                        placeholder={""}
                        className="flex flex-col gap-3 bg-gray-100 dark:bg-gray-300 rounded-md"
                      >
                        {getCompany &&
                          getCompany.map((i) => (
                            <ListItem
